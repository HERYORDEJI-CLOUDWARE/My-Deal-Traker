...
const MortgageSelectedProperty = ({ property, transaction, navigation, route }) => {
  ...
  return (
    <LogoPage navigation={navigation}>
      ...
      <View style={{ borderBottomWidth: 1, paddingBottom: 25 }}>
        ...
        >
          <TouchableOpacity
            activeOpacity={0.9}
            onPress={() =>
              navigate("updateMortgageBroker", {transaction_id:transaction_id, property_id:property_id})
            }
          >
            <Card style={styles.box}>
              <Text style={{fontSize:20, fontWeight: 'bold', color:colors.lightBrown}}>Mortgage Broker</Text>
              <View style={{ position: "absolute", right: 10 }}>
                <AntDesign name="right" />
              </View>
            </Card>
          ...
          <TouchableOpacity
            activeOpacity={0.9}
            onPress={() => navigate("fileUpload", { transaction: transaction_id })}
          >
            <Card style={styles.box}>
              <Text
                style={{
      ...
      {console.log(transaction_id, "this is property_id", property_id)}
    </LogoPage>
  );
};
