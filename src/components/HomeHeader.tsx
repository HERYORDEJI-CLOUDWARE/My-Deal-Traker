...
const HomeHeader = ({
  ...
  const _fetchSubStatus = async () => {
    try {
      ...
      setSubscription(response.data.response.data[0]);
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <RN.View>
          ...
          {user ? (
            <Text style={styles.remaining}>
              Available listing:{" "}
              {subscription?.total_remaining_listings_in_active_plans ?? "0"} /{" "}
              {subscription?.total_allowed_listings_in_active_plans ?? "0"}
            </Text>
          ) : null}
        </RN.View>
        <RN.Image
